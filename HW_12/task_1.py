...
class Student:
    ...
    def __call__(self, lesson: str, number: int, type_est: str = "lesson"):
        ...
        if type_est == "lesson":
            ...
            self.lessons["middle_assessment"] = self.calculate_middle_estimate(
                self.lessons
            )
        elif type_est == "test":
            ...
            self.lessons["lessons"][lesson]["average_test"] = reduce(
                lambda x, y: x + y, self.lessons["lessons"][lesson]["test_results"]
    ...
    def calculate_middle_estimate(lessons: dict) -> float:
        all_estimates = []
        [
            all_estimates.extend(lessons["lessons"][name]["assessments"])
    ...
    def calculate_middle_test(lessons: dict) -> float:
        all_estimates = []
        [
            all_estimates.extend(lessons["lessons"][name]["test_results"])
            for name in lessons["lessons"]
        ]
    ...
    def __repr__(self):
        ...
        for key, value in self.lessons["lessons"].items():
            result += (
                f'{key} = {value["test_results"]}, ср.бал = {value["average_test"]}\n'
            )
